...
// Citation UI parser — regex-matches [Card <id>] and renders clickable badges.
// Matches: \[Card\s+([A-Za-z0-9\-]+)\]
// Text segments render normally; matched IDs render as badges that jump to the card via onView(id).
function renderCitedAnswer(answer, onView) {
  const text = String(answer || '');
  const re = /\[Card\s+([A-Za-z0-9\-]+)\]/g;
  const parts = [];
  let last = 0;
  let m = null;
  let k = 0;
  while ((m = re.exec(text)) !== null) {
    ...
    const id = m[1];
    const label = '[' + String(id).slice(-4) + ']';
    parts.push(htmlR`<button key=${'cite-' + (k++) + '-' + id} onClick=${(e) => { e.preventDefault(); onView(id); }} title=${'View ' + id} className="inline-flex items-center px-1.5 py-0.5 mx-1 text-[10px] font-bold text-blue-700 bg-blue-100 border border-blue-300 rounded cursor-pointer hover:bg-blue-200">${label}</button>`);
...
export function AppRight(p) {
  ...
  // Forward activePersona cleanly into onAsk(question, scopedCards, privacyMode, activePersona)
  // so AiClient.askSmartAssistant() enforces persona-synchronized privacy (no leaks).
  const fireAsk = () => { if (p.onAsk) p.onAsk(p.ask, p.contextCards, p.privacy, p.activePersona); };
  const filterAid = htmlR`<div className="rounded-[12px] bg-[#f0f7ff] border border-[#bfdbfe] p-3"><div className="font-medium text-[12px]">Filter Aid</div><div className="mt-1 text-[10px] italic text-[#6b7280]">Tap a tag to populate the Smart Assistant</div><div className="mt-1 flex flex-wrap gap-1">${aidTags.map((k) => htmlR`<button type="button" key=${k} onClick=${() => p.setAsk(k)} title=${'Filter by ' + k} className="px-2 py-0.5 rounded-full bg-white border text-[11px] hover:border-[#bfdbfe] hover:bg-[#f0f7ff]">${k}</button>`)}</div></div>`;
  return htmlR`<div className="w-full lg:w-[340px] shrink-0 border-l border-[#e5e7eb] bg-white lg:min-h-[calc(100vh-56px)]">
    <div className="p-4 space-y-4">
      <div><div className="font-semibold flex items-center gap-2"><span className="w-4 h-4">✦</span>Smart Assistant</div>
        ...
        <div className="mt-2 flex gap-1 items-center"><div className="relative flex-1"><span className="absolute left-3 top-2.5 w-4 h-4 text-[#9ca3af]">⌕</span><input value=${p.ask} onInput=${(e) => p.setAsk(e.target.value)} onKeyDown=${(e) => { if (e && e.key === 'Enter') { if (e.preventDefault) e.preventDefault(); fireAsk(); } }} placeholder="Ask: MS3, IT support, PO extension..." className="w-full bg-[#f0f7ff] border border-[#bfdbfe] rounded-[10px] pl-9 pr-3 py-2 text-[12px]" /></div><button onClick=${() => { fireAsk(); }} className="px-3 py-2 rounded-[10px] bg-black text-white text-[11px] whitespace-nowrap">Ask Assistant</button></div>
        <div className="mt-2">${filterAid}</div>
        ...
        ${(!p.assistantLoading && p.assistantAnswer) ? htmlR`<div className="mt-2 bg-[#f0f7ff] border border-[#bfdbfe] rounded-[12px] p-3 text-[12px]"><div>${renderCitedAnswer(p.assistantAnswer, p.onView)}</div><div className="mt-2 flex flex-wrap gap-1">${(Array.isArray(p.assistantSources) ? p.assistantSources : []).map((s) => htmlR`<span key=${s} className="px-2 py-0.5 rounded-full bg-[#D6F5E8] border border-[#a7f3d0] text-[10px]">${s}</span>`)}</div><div className="mt-1 text-[10px] italic text-[#6b7280]">${p.privacy === 'My Notes' ? '[Scope: My Notes Only]' : '[Scope: Team Shared Knowledge]'}</div></div>` : null}
        <div className="mt-2"><div className="text-[10px] italic text-[#94A3B8]">Search Results — links to center feed</div><div className="mt-1 space-y-2">${visibleHits.map((d) => htmlR`<div key=${d.id} className="p-2 rounded-[10px] bg-[#f9fafb] border flex justify-between items-center">
