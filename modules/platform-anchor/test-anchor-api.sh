...
# 5. Get anchor
curl -s http://localhost:8000/anchor/Ge%20Aviation%20Uk/GE%20Discovery | python -m json.tool

# 6. List by client PRIMARY FILTER
curl -s http://localhost:8000/anchors/Ge%20Aviation%20Uk | python -m json.tool

# 7. Search by business O-5030460 — users search by this
curl -s http://localhost:8000/search/opportunity/O-5030460 | python -m json.tool

# 8. Search by Salesforce 006Uj... — provenance by URL
curl -s http://localhost:8000/search/connected/006Uj00000QOBkvIAH | python -m json.tool

# 9. Test PII screener middleware
curl -s -X POST http://localhost:8000/test/pii-check -H "Content-Type: application/json" -d '{"text":"Alex Nejat alex@ge.com £129,768 TGS_EmpID 8261003 UK"}' | python -m json.tool
