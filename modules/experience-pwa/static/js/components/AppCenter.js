...
export function AppCenter(p) {
  ...
  // Task 3: re-render on every FailoverDB/VectorSync write (incl. background flush).
  const [, setVecTick] = window.React.useState(0);
  window.React.useEffect(() => {
    const h = () => { try { setVecTick((v) => v + 1); } catch (e) {} };
