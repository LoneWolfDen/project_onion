...
export function findSmartAppendMatch(stagedText, stagedTitle) {
  ...
  // High-value signals nuke immediately; ubiquitous project IDs (O-xxxxxx /
  // OPP-xxxx) are too generic across Apollo/NovaTech cards to auto-append alone.
  // They only count as weak context and still require topic/token confirmation.
  const isHighValueRef = (r) => {
    try {
      const t = String(r || '').toUpperCase();
      if (/^FW-REQ-/i.test(t)) return true;
      if (/^PO-/i.test(t)) return true;
      if (/^SOW-/i.test(t)) return true;
      if (/^AP-/i.test(t)) return true;
  ...
  for (const c of cards) {
    ...
    // THE NUKE: exact high-value Reference ID overlap (e.g. FW-REQ-4471) is an
    // automatic, undeniable match — stop searching, guarantee the append.
    const highValueHit = refOverlap.filter((r) => isHighValueRef(r));
    if (highValueHit.length > 0) { best = c; bestScore = 100; bestReasons = ['Exact ID match: ' + highValueHit[0]]; break; }
    ...
    // Generic O-/OPP- overlap alone is NOT enough — treat as +0 unless backed
    // by topic/title evidence (prevents every Apollo card matching O-008891).
    const genericOnly = refOverlap.length > 0 && highValueHit.length === 0;
    let score = (genericOnly ? 0 : refOverlap.length * 3) + tokOverlap * 1 + (titleHit ? 4 : 0);
    const reasons = [];
    if (refOverlap.length && !genericOnly) reasons.push('ref:' + refOverlap.slice(0, 3).join(','));
    else if (genericOnly) reasons.push('generic ref:' + refOverlap.slice(0, 3).join(',') + ' (needs topic confirm)');
    if (titleHit) reasons.push('title/topic overlap');
    if (tokOverlap >= 2) reasons.push(tokOverlap + ' shared keywords');
    // Loosened threshold: force contextual overlaps to append rather than create isolated noise.
    // Secondary fallback: token/title overlap only (generic refs excluded).
    if (tokOverlap >= 2) score = Math.max(score, 2);
  ...
  return html`<div>
    ...
    <aside id="harvester-control-panel" aria-label="Harvester Control Center" className=${open ? 'open' : ''}>
      ...
      <div className="hcp-scroll"><div id="harvester-target-headline" style=${{ fontSize: '12px', fontWeight: 700 }}>Targeting Ingestion for: ${metaProject}</div>
        ...
        <div className="hcp-card"><div className="hcp-label">Paste Bookmarklet Clipboard String</div><textarea id="harvester-clipboard" rows="4" value=${clip} onInput=${(e) => setClip(e.target.value)} placeholder="Paste Bookmarklet JSON string"></textarea><div style=${{ fontSize: '10px', fontStyle: 'italic', color: '#6b7280' }}>Click the Continuum Bookmarklet button on a live Salesforce or GDP tab, then paste the string here.</div><div style=${{ display: 'flex', gap: '6px', marginTop: '6px', flexWrap: 'wrap' }}><button type="button" onClick=${onStageClipboard} className="px-3 py-1 rounded-full bg-white border text-[11px]">Stage clipboard</button><button type="button" disabled=${processing} onClick=${onRunClipboardHarvest} className="px-3 py-1 rounded-full bg-black text-white text-[11px]">${processing ? 'Harvester running…' : 'Run Harvester & Refine'}</button></div></div>
